...
unsigned char bilinear_interpolation(double u, double v, int row_out, int col_out, int channel, unsigned char pixmap[])
{
  ...
  u0 = (u0 >= (floor(u) + 0.5)) ? (floor(u) + 0.5) : (floor(u) - 0.5);
  v0 = (v0 >= (floor(v) + 0.5)) ? (floor(v) + 0.5) : (floor(v) - 0.5);
  s = u - u0;
  t = v - v0;
  ...
  for (int row_out = 0; row_out < yres_out; row_out++)  // output row
  {
    for (int col_out = 0; col_out < xres_out; col_out++)  // output col
    {
      ...
      if (u <= xres && v <= yres && u >= 0 && v >= 0)
      {
        ...
        for (int k = 0; k < 4; k++)
        {
          // standard inverse map
          // {outputpixmap[(row_out * xres_out + col_out) * 4 + k] = inputpixmap[(row_in * xres + col_in) * 4 + k];}
          // magnification
          if (scale_factor_x < 1 || scale_factor_y < 1)
            {outputpixmap[(row_out * xres_out + col_out) * 4 + k] = bilinear_interpolation(u, v, row_out, col_out, k, super_inputpixmap);}
          // minification
          else
...
command line options parser
  warp input_image_name [output_image_name](optional)
*/
void getCmdOptions(int argc, char* argv[], string &inputImage, string &outputImage)
{
  ...
  {
    cout << "[HELP]" << endl;
    cout << "warp input_image_name [output_image_name]" << endl;
  }
}
